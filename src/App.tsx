...
import { BusinessProfile, AgentMessage, DocFile } from './types';
...
export default function App() {
  ...
  const [solveResponse, setSolveResponse] = useState<{ messages: AgentMessage[]; summary: string } | null>(null);
  ...
  const handleSolveTask = async () => {
    ...
    try {
      const res = await fetch('/api/agents/solve', {
        ...
        body: JSON.stringify({ profile, request: customRequest })
      });
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-amber-500 selection:text-zinc-950">
        ...
        <section className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <label className="block text-[11px] font-mono text-zinc-500 uppercase">Select Target Enterprise Preset</label>
              <div className="grid grid-cols-2 gap-2">
                {BUSINESS_PRESETS.map((bp) => (
                  ...
                  >
                    ...
                    <span className="block font-medium text-xs truncate">{bp.name}</span>
                    <span className="block text-[10px] opacity-70 truncate">{bp.type}</span>
                  </button>
                ))}
              </div>
      ...
      {/* FOOTER BAR */}
      <footer className="mt-20 border-t border-zinc-900 bg-zinc-950 px-6 py-8 flex flex-col md:flex-row items-center justify-between text-zinc-500 text-xs gap-4 max-w-[1600px] mx-auto">
        <div className="flex items-center space-x-2">
          <Shield size={14} />
          <span>Township CEO AI OS • Created for Capstone Showcase • Fully Provider-Independent</span>
        </div>
        <div className="font-mono text-[10px] tracking-wider uppercase text-zinc-600">
          Status: <span className="text-emerald-500">● Core Online</span>
        </div>
      </footer>
